...
<a id="payment-settlement-atomicity"></a>
6. **Payment Settlement Scope & Atomicity (Phase 1 Limitation):** Internal mesh-ledger payments between node0 wallets are 100% end-to-end tested and atomically verified. External Lightning payments routed via LNBits REST API are implemented and replay-protected, but have not yet been end-to-end verified against a live mainnet LNBits instance. Furthermore, external LNBits calls execute before the internal ledger transaction commits; a process crash between settlement and commit requires manual review. A pending-status reconciliation log is planned for Phase 2 — see Section 9.2.
